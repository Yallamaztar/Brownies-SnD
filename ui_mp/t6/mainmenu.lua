...
LUI.createMenu.MainMenu = function (LocalClientIndex)
	...
	modText:setLeftRight(true, false, 4, 295)
	...
	local function TypeWriterTick()
		if currentIndex <= string.len(fullText) then
			local partialText = string.sub(fullText, 1, currentIndex)
			modText:setText(partialText)
			currentIndex = currentIndex + 1
			modText:registerEventHandler("transition_complete_typewriter", TypeWriterTick)
			modText:beginAnimation("typewriter", typingSpeed)
		end
	end
	TypeWriterTick()
	...
	if CoD.isZombie == false then
		...
		local MainMenuBackgroundMP = LUI.UIImage.new()
		MainMenuBackgroundMP:setLeftRight(false, false, -640, 640)
		MainMenuBackgroundMP:setTopBottom(false, false, -360, -180)
		MainMenuBackgroundMP:setZRot(180)
		MainMenuBackgroundMP:setImage(RegisterMaterial("ui_smoke"))
		MainMenuBackgroundMP:setAlpha(0.1)
		MainMenuWidget:addElement(MainMenuBackgroundMP)
	end
